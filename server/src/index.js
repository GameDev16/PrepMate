...
if (!jwtSecret || jwtSecret.length < 32 || jwtSecret.includes("change-in-production")) {
  console.error("FATAL: JWT_SECRET must be at least 32 characters and not match default example strings.");
  process.exit(1);
}
...
app.listen(PORT, () => {
  ...
  if (process.env.GEMINI_API_KEY) {
    console.log(`Gemini API: configured (${process.env.GEMINI_MODEL || "gemini-3-flash-preview"})`);
  } else {
